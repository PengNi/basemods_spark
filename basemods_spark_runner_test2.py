...
def basemods_pipe():
    ...
    metaxml_filenames = []
    cmph5_dir = CMPH5_DATA_DIR
    cmph5_filenames = []
    if DATA_SAVE_MODE == 'MASTER' or DATA_SAVE_MODE == 'SHARED_FOLDER':
        for root, dirnames, filenames in os.walk(cmph5_dir):
            for filename in fnmatch.filter(filenames, '*.aligned_reads.cmp.h5'):
                cmph5_filenames.append(os.path.join(root, filename))
    ...
    print("there are {} cmp.h5 files".format(len(cmph5_filenames)))
    local_temp_dir = TEMP_OUTPUT_FOLDER
    # FIXME: how to do it smarter?---------------
    shuffle_factor = 1000
    numpartitions = len(cmph5_filenames) * shuffle_factor
    re_numpartitions = numpartitions if numpartitions < max_numpartitions else max_numpartitions
    cmph5nameRDD = sc.parallelize(cmph5_filenames, len(cmph5_filenames))\
        .repartition(re_numpartitions)\
        .coalesce(len(cmph5_filenames))

    if DATA_SAVE_MODE == 'MASTER':
        aligned_reads_rdd = cmph5nameRDD.\
            flatMap(split_reads_in_cmph5).\
            partitionBy(len(cmph5_filenames) * reads_shuffle_factor). \
